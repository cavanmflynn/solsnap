...
async function getWalletPlugins(): Promise<{
  [k: string]: { permissionName: string };
}> {
...
export async function isSnapInstalled(pluginOrigin: string): Promise<boolean> {
  try {
    return !!Object.values(await getWalletPlugins()).find(permission => permission.permissionName === pluginOrigin);
  } catch (e) {
